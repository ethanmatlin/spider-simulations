...
def fitness_func(newLoc, spider, F_panel, t, location, locations_of_others, sizes_crosssection, u, num_spiders, xi, kappa):
	...
	if (location[0]<0):
		...
		newLoc_col = newLoc[1]
		return sizes_crosssection[spider]/v(newLoc_row,newLoc_col, t, spider, 
	...
	for t in range(T):
		...
		for j in range(num_spiders):
			...
			if (locations_panel[j,t,0]>=0):
				...
				F_panel[j,t] = fitness_func(locations_panel[j,t], j, F_panel, t, locations_panel[j,t],locations_panel[:,t], sizes_panel[:,t], u, num_spiders, xi, kappa)
		...
		for j in range(num_spiders):
				if (t+1<T):
					...
					# If spider still alive, calculate fitness of potential moves and move to location providing highest expected fitness
						...
						F_calc = [[fitness_func((i, k), j, F_panel, t, locations_panel[j,t],locations_panel[:,t], sizes_panel[:,t], u, num_spiders, xi, kappa) for i in range(num_locations)] for k in range(num_locations)]
...
def graph(meta_parameters):
	...
	+ "smoo=" + boolString(meta_parameters['smooth']))
	...
	plt.savefig('heatmap_t=0' + name_params + '.png', bbox_inches='tight')
	plt.close()

	plt.imshow(number_at_location_time_series[25], cmap='hot', interpolation='nearest')
	plt.savefig('heatmap_t=25' + name_params + '.png', bbox_inches='tight')
	plt.close()

	plt.imshow(number_at_location_time_series[50], cmap='hot', interpolation='nearest')
	plt.savefig('heatmap_t=50' + name_params + '.png', bbox_inches='tight')
	plt.close()

	plt.imshow(number_at_location_time_series[75], cmap='hot', interpolation='nearest')
	plt.savefig('heatmap_t=75' + name_params + '.png', bbox_inches='tight')
	plt.close()

	t = len(sizes_panel[1])-1
	plt.imshow(number_at_location_time_series[t], cmap='hot', interpolation='nearest')
	plt.savefig('heatmap_t=' + str(t) + name_params + '.png', bbox_inches='tight')
	plt.close()


	plt.hist(sizes_panel[:,0])
	plt.savefig('hist_t=0' + name_params + '.png', bbox_inches='tight')
	plt.close()
	plt.hist(sizes_panel[:,25])
	plt.savefig('hist_t=25' + name_params + '.png', bbox_inches='tight')
	plt.close()
	plt.hist(sizes_panel[:,50])
	plt.savefig('hist_t=50' + name_params + '.png', bbox_inches='tight')
	plt.close()
	plt.hist(sizes_panel[:,75])
	plt.savefig('hist_t=75' + name_params + '.png', bbox_inches='tight')
	plt.close()
	plt.hist(sizes_panel[:,99])
	plt.savefig('hist_t=99' + name_params + '.png', bbox_inches='tight')
	plt.close()
